...
@Component({
  selector: 'app-bu-myreqs',
  templateUrl: './bu-myreqs.component.html',
  styleUrls: ['./bu-myreqs.component.scss']
})


export class BuMyreqsComponent implements OnInit {
  Reqs: any;
  form: FormGroup;
  emp_id: string;
  ...
  f_data: any = false;
  fd_data: any = false;
  r_data: boolean = false;
  rd_data: boolean = false;
  
constructor(private http: HttpClient, private routerNavigate: Router, private as: ApiservicesService, private toaster: ToastrService,private spinner: NgxSpinnerService) {
...
addInternalreq(internData) {
    ...
    this.internalreqlist();
    internData.resetForm({user_id:this.user_id});
    
    });
    this.routerNavigate['bu/bumyreqs']
  }
